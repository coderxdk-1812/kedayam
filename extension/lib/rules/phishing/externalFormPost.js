...
export const ruleExternalFormPost = Object.freeze({
  ...
  evaluate(ctx) {
    ...
    const pageRoot = ctx.pageRoot || (ctx.pageOrigin ? rootDomain(new URL(ctx.pageOrigin).host) : null);
    ...
    for (const f of forms) {
      ...
      try {
        ...
        if (ar && ar !== pageRoot) {
          return {
            matched: true, contribution: -60,
            explain: `Credential form on ${pageRoot} posts to ${ar}.`,
          };
        }
      } catch { /* invalid action — handled elsewhere */ }
    }
