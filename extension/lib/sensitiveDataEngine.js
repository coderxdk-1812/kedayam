...
const PATTERNS = [
  ...
  },
  {
    ...
    minEntropy: 4.0,
    contextHints: /aws|secret|amazon|amzn/i,
    confidence: 0.7,
  },
  {
    id: "gcp_api_key",
  ...
  },
  {
    ...
    contextHints: /azure|blob|storage|core\.windows\.net/i,
    confidence: 0.7,
  },
  ...
  {
    id: "ssh_authorized",
    label: "SSH public key (review)",
    severity: "low",
    regex: /\bssh-(?:rsa|ed25519|dss) [A-Za-z0-9+/=]{40,}/g,
    confidence: 0.8,
  },
  {
    id: "jwt",
  ...
  },
  {
    ...
    validate: (s) => looksLikeMnemonic(s),
    confidence: 0.95,
  },
  ...
  },
  {
    ...
    contextHints: /swift|bic|bank|wire/i,
    confidence: 0.7,
  },
  ...
  { id: "pan", label: "PAN", severity: "high", regex: /\b[A-Z]{5}\d{4}[A-Z]\b/g, confidence: 0.9 },

  // --- MFA / recovery ---
  {
    id: "mfa_backup_code",
    label: "MFA backup code",
    severity: "high",
...
export function analyzeSensitivePayload(text, meta = {}) {
  ...
  for (const tok of (normalized.match(/[A-Za-z0-9+/_=-]{32,}/g) || []).slice(0, 40)) {
    ...
    findings.push({
      id: "entropy_blob",
      label: "High-entropy secret-like blob",
      severity: "medium",
      confidence: round2(Math.min(0.85, (H - 4.5) * 0.5 + 0.55)),
      redacted: redact(tok),
      offset: normalized.indexOf(tok),
      suppressed: false,
    });
  }
...
function emptyVerdict() {
  return { findings: [], detectedTypes: [], riskScore: 0, riskLevel: "none", suppressed: 0 };
}
