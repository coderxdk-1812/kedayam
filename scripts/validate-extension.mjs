...
function validRelative(rel, label) {
  ...
  if (rel.startsWith("/") || rel.includes("..") || normalize(rel).startsWith(".."))
    errors.push(`${label} must stay inside the extension package: ${rel}`);
  exists(rel);
}
function iconMap(map, label) {
  for (const size of ["16", "32", "48", "128"]) {
    if (!map?.[size]) errors.push(`${label} is missing ${size}px icon`);
      ...
      if (!map[size].endsWith(".png"))
        errors.push(`${label}.${size} must be a PNG file, not ${map[size]}`);
    }
  }
}
...
if (!manifest.name || manifest.name.length > 75)
  errors.push("name is required and must be <= 75 characters");
if (!/^\d+\.\d+\.\d+(\.\d+)?$/.test(manifest.version || ""))
  errors.push("version must be Chrome-compatible semver");
if (!manifest.description || manifest.description.length > 132)
  warnings.push("description should be present and <= 132 characters for the Chrome Web Store");
...
if (manifest.background?.type !== "module")
  errors.push('background.type must be "module" for this ES module service worker');
...
for (const [i, cs] of (manifest.content_scripts || []).entries()) {
  ...
  if (!Array.isArray(cs.matches) || !cs.matches.length)
    errors.push(`content_scripts[${i}].matches is required`);
}
...
for (const file of jsFiles) {
  ...
  const rx = [
    ...
    {
      re: /\.textContent\s*=\s*[`'"][^`'"\n]*function/g,
      msg: "looks like inline-script injection via textContent",
    },
    {
      re: /innerHTML\s*=\s*[`'"][^`'"]*<script/gi,
      msg: "injects <script> via innerHTML (CSP-violating)",
    },
  ];
...
console.log(
  `Kedayam extension valid. Checked manifest, ${jsFiles.length} JS/HTML files for CSP violations, and ${manifest.web_accessible_resources?.length || 0} WAR groups.`,
);
