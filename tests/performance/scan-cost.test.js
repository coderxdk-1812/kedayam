...
describe("performance — scan cost", () => {
  ...
  it("phishing heuristics: <10ms on a typical login ctx", () => {
    const ctx = {
      ...
      forms: [
        { hasPassword: true, hasEmailLike: true, hasOtp: false, hiddenCount: 1, fieldsCount: 3 },
      ],
    ...
    expect(
      time(() => {
        for (let i = 0; i < 50; i++) analyzePhishing(ctx);
      }),
    ).toBeLessThan(150);
  });
  ...
  it("arbitration: deterministic <2ms even on a dense input", () => {
    ...
    expect(
      time(() => {
        for (let i = 0; i < 200; i++) arbitrate(ctx);
      }),
    ).toBeLessThan(50);
  });
  ...
  it("evaluateUrl: full pipeline mean <50ms across 20 URLs", async () => {
    const urls = [
      ...
      "https://wikipedia.org",
      "https://m1cros0ft.example.org/login",
      "https://paypa1.example/login",
      "https://verify-now.example.cc",
      "https://signin.bad-host.example",
