  ...
  it("password field injected after first render is still classified as auth", () => {
    const ctx = withForms({
      forms: [{ hasPassword: true, hasEmailLike: true, hasOtp: false,
        hiddenCount: 0, fieldsCount: 2, action: "https://attacker.example.com/x",
        method: "post" }],
      hasPasswordField: true, firstFieldKind: "email",
    });
  ...
  it("obfuscated input names (random ids) still trigger auth detection", () => {
    const ctx = withForms({
      forms: [{ hasPassword: true, hasEmailLike: false, hasOtp: false,
        hiddenCount: 4, fieldsCount: 6, action: "", method: "post" }],
  ...
  it("dynamically swapped form action still flagged when off-domain", () => {
    const ctx = withForms({
      ...
      forms: [{ hasPassword: true, hasEmailLike: true, hasOtp: false,
        ...
        action: "https://collector.elsewhere.tld/steal", method: "post" }],
      hasPasswordField: true, firstFieldKind: "email",
