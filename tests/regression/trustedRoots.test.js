...
const TRUSTED = [
  ...
  },
  {
    ...
    ctx: brandPageContext("https://login.microsoftonline.com", "https://aadcdn.msftauth.net", {
  ...
  {
    name: "Okta",
    url: "https://acme.okta.com/login/login.htm",
...
describe("REGRESSION: trusted root + behavioral evidence still escalates", () => {
  it("github.com with off-domain credential POST IS dangerous", async () => {
    const ctx = brandPageContext("https://github.com", "https://github.githubassets.com");
    ctx.forms = [
      {
        action: "https://attacker.tld/collect",
        method: "post",
        hasPassword: true,
        hasEmailLike: true,
        hasOtp: false,
    ...
    const r = await evaluateUrl("https://github.com/login", { settings, pageContext: ctx });
